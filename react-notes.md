...
* __Creating redux__ _(uses `@reduxjs/toolkit` library)_
    1. `createSlice` create a slice for each state which will provide reducer, actions and selectors for the application.
...
### creating slice
slice provides all features to manage state.  

createSlice contains object including
...
```
Major roles of slice
1. Generates action types and creators based on reducer function name.
2. Combine multiple reducer functions into one reducer function.
3. Use Immer library to enable state mutation while tracking the changes.

__action generators or action creators__  
1. action creators are functions that returns action object with type and payload.
2. `createSlice` creates action creator functions with same name as reducers functions and they are available in `sliceName.actions`
    * `increment` and `decrement` are two action creators generated by createSlice.
3. If we execute action creators it gives object contaning
    * __type__ sliceName/actionName  
     _counter/increment_ or _counter/decrement_
    * __payload__ parameter passed while executing aciton creators 

### creating store
create common space for all states.  
configureStore contains an object including:
1. `reducer` _(required)_ reducers for all state and combine into one
2. `middleware` (_optional_) by default thunk is added.
...
### accessing state in application
`useSelector`
